namespace BigGustave
{
    ...
    public struct ImageHeader
    {
        ...
        public ImageHeader(int width, int height, byte bitDepth, ColorType colorType, CompressionMethod compressionMethod, FilterMethod filterMethod, InterlaceMethod interlaceMethod)
        {
            if (width == 0)
            {
                throw new ArgumentException(nameof(width), "Invalid width (0) for image.");
            }

            if (height == 0)
            {
                throw new ArgumentException(nameof(height), "Invalid height (0) for image.");
            }

            HashSet<byte> permitted;
            if (!PermittedBitDepths.TryGetValue(colorType, out permitted)
