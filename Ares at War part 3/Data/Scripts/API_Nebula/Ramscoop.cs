	...
	public class NebulaRamscoop : MyGameLogicComponent{
		...
		public override void UpdateBeforeSimulation10(){
			...
			try{
				...
				Position = Ramscoop.GetPosition();

				var Orientation = Ramscoop.WorldMatrix.GetDirectionVector(Base6Directions.Direction.Forward);
				var RayOrigin = Position + (5 * Orientation);
				var RayTarget = RayOrigin + (100 * Orientation);
					
				float Grav;
				float Density = Jakaria.API.NebulaAPI.GetNebulaDensity(Ramscoop.GetPosition());
				//float Density = 1;
				...
				if (!Jakaria.API.NebulaAPI.InsideNebulaBounding(Ramscoop.GetPosition())){
