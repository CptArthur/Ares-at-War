	...
	public class SavedInternalData {
		...
		public static SavedInternalData LoadSettings(string phase) {
			...
			if (MyAPIGateway.Utilities.FileExistsInLocalStorage("SavedInternalData-" + Settings.UniqueModId + ".mes", typeof(SavedInternalData)) == true) {
				...
				try {
					...
					Logger.Write("Loaded Existing Settings From SavedInternalData.mes. Phase: " + phase, SpawnerDebugEnum.Startup, true);
					return config;

				} catch (Exception exc) {

					Logger.Write("ERROR: Could Not Load Settings From SavedInternalData.mes. Using Default Data. Phase: " + phase, SpawnerDebugEnum.Error, true);
				...
				Logger.Write("SavedInternalData.mes Doesn't Exist. Creating Default Data. Phase: " + phase, SpawnerDebugEnum.Startup, true);
			...
			try {
				...
				Logger.Write("ERROR: Could Not Create SavedInternalData.mes. Default Data Will Be Used. Phase: " + phase, SpawnerDebugEnum.Error, true);
		...
		public string SaveSettings() {
			...
			try {
				...
				Logger.Write("Settings In SavedInternalData.mes Updated Successfully!", SpawnerDebugEnum.Settings);
				return "Settings Updated Successfully.";

			} catch (Exception exc) {

				Logger.Write("ERROR: Could Not Save To SavedInternalData.mes. Changes Will Be Lost On World Reload.", SpawnerDebugEnum.Settings);
